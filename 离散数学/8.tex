...
\begin{itemize}
	...
	\begin{itemize}
		\item 公理模式 $\mathscr{A}_1$（肯定后件律）：$Q \to (R \to Q)$；
		\item 公理模式 $\mathscr{A}_2$（蕴含词分配律）：$(P \to (Q \to R)) \to ((P \to Q) \to (P \to R))$；
		\item 公理模式 $\mathscr{A}_3$（换位律）：$(\lnot Q \to \lnot R) \to (R \to Q)$。
	\end{itemize}
	\item 推理规则（分离规则，Modus Ponents 规则）：
	\begin{itemize}
