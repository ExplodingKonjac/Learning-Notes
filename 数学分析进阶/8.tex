...
\begin{problem}
    习题 8.4.1
    \begin{solution}
        由题可知，所给直线的方向向量为 $(2, 1, 5)$。因此所求直线方程为
        $$
        (x, y, z) = (4, -1, 3) + \lambda (2, 1, 5)
        $$
    \end{solution}
\end{problem}
...
\begin{problem}
    习题 8.4.4
    \begin{solution}
        该直线是两个平面的交，这两个平面的法向量为
        $$
        \vect{n}_1 = (1, -2, 4),\ \vect{n}_2 = (3, 5, -2)
        $$
        因此该直线有一个方向向量
        $$
        \vect{v} = \vect{n}_1 \cp \vect{n}_2 = (-16, 14, 11)
        $$
        所以所求的平面方程为
        $$
        -16 (x - 2) + 11 (z + 3) = 0
...
\begin{problem}
    ...
    \begin{proof}
        第一个直线的一个方向向量为
        $$
        \vect{v}_1 = (1, 2, -1) \cp (-2, 1, 1) = (3, 1, 5)
        $$
        第二个直线的一个方向向量为
        $$
        \vect{v}_2 = (3, 6, -3) \cp (2, -1, -1) = (-9, -3, -15)
        $$
        显然有 $\vect{v}_1 \operatorname{//} \vect{v}_2$，得证。
    \end{proof}
\end{problem}
...
\begin{problem}
    习题 8.4.8
    \begin{solution}
        由题可得，给出的直线过点 $(4, -3, 0)$，且方向向量为 $(5, 2, 1)$。因此所求平面的法向量
        $$
        \vect{n} = (5, 2, 1) \cp \ab((3, 1, -2) - (4, -3, 0)) = (-8, 9, 22)
        $$
        因此所求平面方程为
        $$
...
\begin{problem}
    ...
    \begin{solution}
        \begin{enumerate}
            \item[\textbf{1)}] 直线方向向量为 $\vect{v} = (-2, -7, 3)$，平面法向量为 $\vect{n} = (4, -2, -2)$，有 $\vect{v} \vdot \vect{n} = 0$。因此直线与平面平行。
            \item[\textbf{2)}] 直线方向向量为 $\vect{v} = (3, -2, 7)$，平面法向量为 $\vect{n} = (3, -2, 7)$，有 $\vect{v} = \vect{n}$。因此直线与平面垂直。
            \item[\textbf{3)}] 直线方向向量为 $\vect{v} = (3, 1, -4)$，平面法向量为 $\vect{n} = (1, 1, 1)$。直线与平面相交。
        \end{enumerate}
    \end{solution}
\end{problem}
...
\begin{problem}
    习题 8.4.12
    \begin{solution}
        该平面的法向量为 $\vect{n} = (1, 2, -1)$。假设投影的点为 $(-1, 2, 0) + \lambda \vect{n}$，那么：
        $$
        (-1 + \lambda) + 2 (2 + 2 \lambda) - (-\lambda) + 1 = 0
        $$
        解得 $\lambda = -1$。因此投影点为 $(-2, 0, -1)$。
    \end{solution}
\end{problem}