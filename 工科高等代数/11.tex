...
那么二次型可以表示为：
$$
f(\vect{x}) = \vect{x}^\mathrm{T} \matr{A} \vect{x}
$$
...
\begin{property}
	...
	\begin{proof}
		设有二次型 $f(\vect{x}) = \vect{x}^\mathrm{T} \matr{A} \vect{x}$ 和线性替换 $\vect{x} = \matr{C} \vect{y}$，那么：
		$$
		f(\vect{x}) = (\matr{C} \vect{y})^\mathrm{T} \matr{A} (\matr{C} \vect{y}) = \vect{y}^\mathrm{T} (\matr{C}^\mathrm{T} \matr{A} \matr{C}) \vect{y}
		$$
	\end{proof}
\end{property}
...
\begin{definition}[合同变换]
	...
	$$
	\matr{B} = \matr{C}^\mathrm{T} \matr{A} \matr{C}
	$$
	那么变换 $\matr{A} \mapsto \matr{C}^\mathrm{T} \matr{A} \matr{C}$ 称为矩阵的一个\textbf{合同变换}。
\end{definition}

合同关系具有等价关系的性质：

\begin{itemize}
	\item 自反性：$\matr{A} = \matr{E}^\mathrm{T} \matr{A} \matr{E}$；
	\item 对称性：$\matr{B} = \matr{C}^\mathrm{T} \matr{A} \matr{C} \Leftrightarrow \matr{A} = (\matr{C}^{-1})^\mathrm{T} \matr{B} \matr{C}^{-1}$；
	\item 传递性：$\matr{A}_1 = \matr{C}_1^\mathrm{T} \matr{A} \matr{C}_1 \land \matr{A}_2 = \matr{C}_2^\mathrm{T} \matr{A}_1 \matr{C}_2 \Rightarrow \matr{A}_2 = (\matr{C}_1 \matr{C}_2)^\mathrm{T} \matr{A} (\matr{C}_1 \matr{C}_2)$。
\end{itemize}