...
\begin{property}[定积分的线性性]
	...
	$$
	\int_a^b \ab(\alpha f(x) + \beta g(x)) \dif x = \alpha \int_a^b f(x) \dif x + \beta \int_a^b g(x) \dif x
	$$
\end{property}
...
\begin{property}[估值不等式]
	...
	$$
	m(b - a) \le \int_a^b f(x) \dif x \le M(b - a)
	$$
\end{property}
...
\begin{property}[积分区间可加性]
	...
	\begin{proof}
		...
		下面再证明 $\int_a^b f(x) \dif x = \int_a^c f(x) \dif x + \int_c^b f(x) \dif x$。

		\begin{itemize}
			\item 若 $c \in [a, b]$：此时可以取 $c$ 为第 $k$ 个分割点，那么：
			$$
			\begin{aligned}
				\int_a^b f(x) \dif x & = \lim_{\lambda \to 0} \sum_{i=1}^n f(\xi_i) |\Delta_i| \\
				& = \lim_{\lambda \to 0} \sum_{i=1}^k f(\xi_i) |\Delta_i| + \lim_{\lambda \to 0} \sum_{i=k+1}^n f(\xi_i) |\Delta_i| \\
				& = \int_a^c f(x) \dif x + \int_c^b f(x) \dif x
			\end{aligned}
			$$
			...
			\begin{aligned}
				...
				\int_a^b f(x) \dif x & = \int_a^c f(x) \dif x - \int_b^c f(x) \dif x \\
				& = \int_a^c f(x) \dif x + \int_c^b f(x) \dif x
			\end{aligned}
			$$
		\end{itemize}
