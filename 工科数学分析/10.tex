...
要求隐函数的导数，可以对关系式两边同时对 $x$ 求导，得到关系式 $A(x, y) \frac{\dif y}{\dif x} = B(x, y)$，然后求出 $\frac{\dif y}{\dif x} = \frac{B(x, y)}{A(x, y)}$。
...
\begin{example}
	...
	\begin{solution}
		两边同时对 $x$ 求导得到：
		$$
		\frac{\dif y}{\dif x} - 1 - \varepsilon \cos y \frac{\dif y}{\dif x} = 0
		$$
		解得
		$$
		\frac{\dif y}{\dif x} = \frac{1}{1 - \varepsilon \cos y}
		$$
	\end{solution}
\end{example}
...
$$
\frac{\dif y}{\dif x} = \frac{\frac{\dif y}{\dif t}}{\frac{\dif x}{\dif t}} = \frac{\psi'(t)}{\varphi'(t)} = \frac{\psi'(\varphi^{-1}(x))}{\varphi'(\varphi^{-1}(x))}
$$
...
\begin{example}
	...
	\begin{solution}
		$$
		\begin{aligned}
			\frac{\dif y}{\dif x} & = \frac{3a \sin^2 t \cos t}{-3a \cos^2 t \sin t} \\
			& = -\tan t \\
