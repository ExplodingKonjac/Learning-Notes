...
要求隐函数的导数，可以对关系式两边同时对 $x$ 求导，得到关系式 $A(x, y) \dv{y}{x} = B(x, y)$，然后求出 $\dv{y}{x} = \frac{B(x, y)}{A(x, y)}$。
...
\begin{example}
	...
	\begin{solution}
		两边同时对 $x$ 求导得到：
		$$
		\dv{y}{x} - 1 - \varepsilon \cos y \dv{y}{x} = 0
		$$
		解得
		$$
		\dv{y}{x} = \frac{1}{1 - \varepsilon \cos y}
		$$
	\end{solution}
\end{example}
...
$$
\dv{y}{x} = \frac{\dv{y}{t}}{\dv{x}{t}} = \frac{\psi'(t)}{\varphi'(t)} = \frac{\psi'(\varphi^{-1}(x))}{\varphi'(\varphi^{-1}(x))}
$$
...
\begin{example}
	...
	\begin{solution}
		$$
		\begin{aligned}
			\dv{y}{x} & = \frac{3a \sin^2 t \cos t}{-3a \cos^2 t \sin t} \\
			& = -\tan t \\
