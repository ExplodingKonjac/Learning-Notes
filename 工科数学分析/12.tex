...
\begin{problem}
	...
	\begin{solution}
		\begin{enumerate}
			...
			$$
			\ab(\dv{y}{x} + 1) \sec^2 (x + y) - y - x \dv{y}{x} = 0
			$$
			解得
			$$
			\dv{y}{x} = \frac{y \cos^2 (x + y) - 1}{1 - x \cos^2 (x + y)}
			$$
			再求一次导得到：
			$$
...
\begin{problem}
	...
	\begin{solution}
		\begin{enumerate}
			\item[\textbf{2)}] 由题得到：
			$$
			\dv{x}{t} = -at \sin t + a \cos t,\ \dv{y}{x} = at \cos t + a \sin t
			$$
			那么：
			$$
			\dv{y}{x} = \frac{at \cos t + a \sin t}{-at \sin t + a \cos t}
			$$
			于是得到：
			$$
			\begin{aligned}
				\frac{\dd^2 y}{\dd x^2} & = \frac{\dd}{\dd t} \ab(\frac{t \cos t + \sin t}{-t \sin t + \cos t}) \cdot \dv{t}{x} \\
