...
这也就说明，导数的记号 $\dv{y}{x}$ 可以看作一个整体，也可以看作 $y, x$ 的微分之商。因此导数也被称做微商。
...
\begin{itemize}
	\item $\dd (u + v) = \dd u + \dd v$；
	\item $\dd (u - v) = \dd u - \dd v$；
	\item $\dd (C u) = C \dd u$；
	\item $\dd (u \cdot v) = u \dd v + v \dd u$；
	\item $\dd \frac{u}{v} = \frac{v \dd u - u \dd v}{v^2}$；
\end{itemize}
...
\begin{definition}[函数的高阶微分]
	...
	\begin{gathered}
		\dd y = y' \dd x \\
		\dd^2 y = \dd (y' \dd x) = y'' \dd x^2 \\
		\cdots \\
		\dd^n y = y^{(n)} \dd x^n
	\end{gathered}
	$$
	这里面出现 $\dd x$ 的高次幂的原因是，在求 $y' \dd x$ 微分时，$\dd x$ 被视作常数。因此 $\dd(y' \dd x) = \dd(y'') \dd x = y'' \dd x^2$。更高阶也同理。
\end{definition}
...
特别地，在 $x_0 = 0$ 处的 Taylor 展开式被称为\textbf{Maclaurin 展开式}。下面是一些常见的 Maclaurin 展开式：
