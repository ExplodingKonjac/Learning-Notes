...
$$
A = \int_{t_1}^{t_2} |\psi(t) \,\dif \varphi(t)| = \int_{t_1}^{t_2} |\psi(t) \varphi'(t)| \,\dif t
$$
...
\begin{figure}[H]
	...
	\begin{tikzpicture}[scale=2]
		...
		\node at (52.5:1.0) {$\dif \theta$};

		% Label the curve
		\node at (2.4, 0.5) {$r = \varphi(\theta)$};
		
		% Label the area element
		\node at (1.2, 1.6) {$\dif A \approx \frac{1}{2} r^2 \dif \theta$};
	\end{tikzpicture}
...
得到图形面积为：
$$
A = \int_\alpha^\beta \frac{1}{2} \varphi(\theta)^2 \,\dif \theta
$$
...
\begin{figure}[H]
	...
	\begin{tikzpicture}[scale=1.5]
		...
		\node[below, font=\footnotesize] at (\xslice+0.5*\dx, -0.5) {$\dif x$};
		...
		\draw[->] (3.2, 1.5) node[right, align=left] {$\dif V \approx \pi f(x)^2 \dif x$} to[out=180, in=45] (\xslice+0.5*\dx, 0.8*\rnext);
...
\begin{tikzpicture}[scale=1.5, >=stealth]
		...
		% dx and dy sides
		\draw[dashed] (A) -- (C) node[midway, below] {$\dif x$};
		\draw[dashed] (C) -- (B) node[midway, right] {$\dif y$};
		...
		\node[blue, above right] at (\xa + 0.3*\dx, \ya + 0.4) {$\dif s$};
		...
		\draw[dashed, thin, gray] (C) -- (\xb, 0) node[below, black] {$x+\dif x$};
...
得到弧长为：
$$
s = \int_a^b \dif s = \int_a^b \sqrt{\dif y^2 + \dif x^2} = \int_a^b \sqrt{1 + f'(x)^2} \dif x
$$
...
使用类似的微元分析，可以得到：
$$
s = \int_a^b \sqrt{\dif \varphi(t)^2 + \dif \psi(t)^2} = \int_a^b \sqrt{\varphi'(t)^2 + \psi'(t)^2} \,\dif t
$$
...
于是得到弧长为：
$$
s = \int_\alpha^\beta \sqrt{f(\theta)^2 + f'(\theta)^2} \, \dif \theta
$$
...
\begin{problem}
	...
	\begin{solution}
		\begin{enumerate}
			\item[\textbf{2)}] 由题可知：
			$$
			\int_0^x \E^{t^2} \,\dif t > x \cdot \E^0 = x \\
			\int_0^x \E^{2 t^2} \,\dif t > x \cdot \E^0 = x
			$$
			因此 $x \to \infty$ 时，$\int_0^x \E^{t^2} \,\dif t \to \infty,\ \int_0^x \E^{2 t^2} \,\dif t \to \infty$。

			对于原题，利用 L'Hospital 法则：
			$$
			\begin{aligned}
				\text{原式} & = \lim_{x \to \infty} \frac{2 \E^{x^2} \int_0^x \E^{t^2} \,\dif t}{\E^{2 x^2}} = \lim_{x \to \infty} \frac{2 \int_0^x \E^{t^2} \,\dif t}{\E^{x^2}} \\
...
\begin{problem}
	...
	\begin{solution}
		...
		\begin{enumerate}
			\item[\textbf{2)}]
			$$
			\text{原式} = \int_{1/4}^{1/2} 2 \arcsin \sqrt{x} \,\dif \arcsin \sqrt{x} = \ab[\ab(\arcsin \sqrt{x})^2]_{1/4}^{1/2} = \frac{5 \pi^2}{144}
			$$
			\item[\textbf{4)}]
			$$
			...
			$$
			\begin{aligned}
				\text{原式} & = \int_0^{\pi/2} a^2 \sin^2 t \cdot a \cos t \cdot a \cos t \,\dif t \\
				& = a^4 \int_0^{\pi/2} \sin^2 t \cos^2 t \,\dif t \\
				& = \ab[\frac{a^4}{4} \ab(\frac{1}{2} t - \frac{1}{8} \sin 4t)]_0^{\pi/2} = \frac{a^4 \pi}{16}
			\end{aligned}
			$$
			\item[\textbf{8)}] 令 $x \gets \sec t$，那么：
			$$
			\text{原式} = \int_{2\pi/3}^{\pi} \frac{1}{\sec t \tan t} \cdot \sec t \tan t \,\dif t = [t]_{2\pi/3}^{\pi} = \frac{\pi}{3}
			$$
			\item[\textbf{10)}]
			$$
			\begin{aligned}
				\text{原式} & = \int_0^2 \frac{1 + \frac{1}{x^2}}{x^2 + \frac{1}{x^2}} \, \dif x = \int_0^2 \frac{\dif \ab(x - \frac{1}{x})}{\ab(x - \frac{1}{x})^2 + 2} \\
...
\begin{problem}
	...
	\begin{solution}
		\begin{enumerate}
			...
			$$
			\begin{aligned}
				\text{原式} & = 2 \int_0^{\pi/2} \sqrt{\sin^3 x (1 - \sin^2 x)} \,\dif x \\
				& = 2 \int_0^{\pi/2} \sqrt{\sin^3 x \cos^2 x} \,\dif x \\
				& = 2 \int_0^{\pi/2} \sin^{\frac{3}{2}} x \cos x \,\dif x \\
				& = 2 \int_0^{\pi/2} \sin^{\frac{3}{2}} x \,\dif (\sin x) \\
			...
			\item[\textbf{4)}]
			$$
			\text{原式} = \int_{1/\E}^1 -\ln x \,\dif x + \int_1^{\E} \ln x \,\dif x = \E + \frac{1}{\E} - 2
			$$

			\item[\textbf{6)}]
			$$
			\begin{aligned}
				\int x (\arctan x)^2 \,\dif x & = \frac{1}{2} (\arctan x)^2 \,\dif (x^2) \\
				& = \frac{1}{2} x^2 (\arctan x)^2 - \int \frac{x^2}{1 + x^2} \arctan x \,\dif x \\
				& = \frac{1}{2} x^2 (\arctan x)^2 - \int \arctan x \,\dif x + \int \frac{\arctan x}{1 + x^2} \,\dif x \\
			...
			\item[\textbf{10)}]
			$$
			\text{原式} = \int_0^2 \sqrt{4 - x^2} \,\dif \ab(\sqrt{4 + x^2})
			$$
			注意到 $\ab(\sqrt{4 - x^2})^2 + \ab(\sqrt{4 + x^2})^2 = 8$。于是令 $\sqrt{4 - x^2} = 2 \sqrt{2} \cos t,\ \sqrt{4 + x^2} = 2 \sqrt{2} \sin t$，那么：
			$$
			\begin{aligned}
				\text{原式} & = 8 \int_{\pi/4}^{\pi/2} \cos t \,\dif (\sin t) = 8 \int_{\pi/4}^{\pi/2} \cos^2 t \,\dif t \\
				& = 8 \int_{\pi/4}^{\pi/2} \frac{1 + \cos 2t}{2} \,\dif t \\
...
\begin{problem}
	...
	\begin{proof}
		$$
		\begin{aligned}
			\frac{1}{\E} = \ab[f(x) \E^{-x}]_0^1 = \int_0^1 \ab(f'(x) - f(x)) \E^{-x} \,\dif x \le \int_0^1 |f'(x) - f(x)| \E^{-x} \,\dif x \le \int_0^1 |f'(x) - f(x)| \,\dif x
		\end{aligned}
		$$
	\end{proof}
\end{problem}
...
\begin{problem}
	...
	\begin{proof}
		...
		首先根据题目条件和定积分的线性性，有：
		$$
		\int_a^b \ab(\sum_{i=1}^n a_i x^i) f(x) \,\dif x = 0
		$$
...
\begin{problem}
	...
	\begin{solution}
		...
		$$
		\begin{aligned}
			A & = \int_{-2 \sqrt{p(R - p)}}^{2 \sqrt{p(R - p)}} \ab(2(R - p) - \frac{y^2}{2p}) \,\dif y \\
...
\begin{problem}
	...
	\begin{solution}
		...
		$$
		A = \frac{1}{2} \cdot \sqrt{t} \cdot 2t - \int_0^t \sqrt{x} \,\dif x = t^{\frac{3}{2}} - \ab[\frac{2}{3} x^\frac{3}{2}]_0^t = \frac{1}{3} t^{\frac{3}{2}}
		$$
...
\begin{problem}
	...
	\begin{solution}
		...
		因为曲线的对称性，我们先计算 $t \in [0, \frac{\pi}{2}]$ 的面积。考虑使用微元法。当 $t$ 有微小增量 $\dif t$ 时，有：
		$$
		\dif V = \frac{1}{2} (x (y + \dif y) - (x + \dif x) y) = \frac{1}{2} (x \dif y - y \dif x)
		$$

		那么：
		$$
		\begin{aligned}
			A_1 & = \int_0^{\pi/2} a \cos^3 t \cdot 3a \cos t \sin^2 t \,\dif t \\
			& = 3a^2 \int_0^{\pi/2} \cos^4 t \sin^2 t \,\dif t \\
			& = 3a^2 \int_0^{\pi/2} \cos^4 t (1 - \cos^2 t) \,\dif t \\
			& = 3a^2 \ab(\int_0^{\pi/2} \cos^4 t \,\dif t - \int_0^{\pi/2} \cos^6 t \,\dif t) \\
			& = 3a^2 \ab(\frac{3 \cdot 1}{4 \cdot 2} \cdot \frac{\pi}{2} - \frac{5 \cdot 3 \cdot 1}{6 \cdot 4 \cdot 2} \cdot \frac{\pi}{2}) = \frac{3 \pi}{32} a^2 \\
			A_2 & = \int_0^{\pi/2} a \sin^3 t \cdot \ab(-3a \sin t \cos^2 t) \,\dif t = -\frac{3 \pi}{32} a^2 
		\end{aligned}
		$$
		最终整个图形的面积为：
		$$
...
\begin{problem}
	...
	\begin{solution}
		...
		$$
		\begin{aligned}
			A' & = \int_0^{\pi/2} \frac{1}{2} \rho^2 \,\dif \theta = \frac{a^2}{2} \int_0^{\pi/2} \sin^2 2\theta \,\dif \theta \\
			& = \frac{a^2}{2} \int_0^{\pi/2} \frac{1 - \cos 4\theta}{2} \,\dif \theta \\
...
\begin{problem}
	...
	\begin{solution}
		$$
		V = \int_0^8 \pi y^{\frac{2}{3}} \,\dif y = \pi \ab[\frac{3}{5} y^{\frac{5}{3}}]_0^8 = \frac{32}{5} \pi
		$$
	\end{solution}
\end{problem}
...
\begin{problem}
	...
	\begin{solution}
		先考虑 $x \ge 0$ 的部分，此时 $t \in [0, \frac{\pi}{2}]$。对 $x$ 轴方向做微元法，那么在 $t$ 有微小增量 $\dif t$ 时，圆盘半径为 $y$，高为 $\dif x$，于是有：
		$$
		\begin{aligned}
			V' & = \int_0^{\pi/2} \pi \ab(a \sin^3 t)^2 \cdot 3a \sin t \cos^2 t \dif t \\
			& = 3a^3 \int_0^{\pi/2} \sin^7 t \cos^2 t \dif t \\
			& = 3a^3 \int_0^{\pi/2} \sin^7 t (1 - \sin^2 t) \dif t \\
			& = 3a^3 \ab(\int_0^{\pi/2} \sin^7 t \dif t - \int_0^{\pi/2} \sin^9 t \dif t) \\
...
\begin{problem}
	...
	\begin{solution}
		$$
		\begin{aligned}
			V & = \int_{-1}^1 \pi \ab(\ab(2 + \sqrt{1 - y^2})^2 - \ab(2 - \sqrt{1 - y^2})^2) \,\dif y \\
			& = \pi \int_{-1}^1 8 \sqrt{1 - y^2} \,\dif y \\
			& = 8 \pi \cdot \pi = 8\pi^2
		\end{aligned}
		$$
	\end{solution}
...
\begin{problem}
	...
	\begin{solution}
		...
		\begin{itemize}
			\item 对于切线围出的部分：
			$$
			V_1 = \int_0^\E \pi \ab(\frac{1}{\E} x)^2 \,\dif x = \frac{\pi}{\E^2} \ab[\frac{1}{3} x^3]_0^\E = \frac{\pi \E}{3}
			$$
			\item 对于 $y = \ln x$ 围出的部分：
			$$
			V_2 = \int_1^\E \pi \ln^2 x \,\dif x = \pi \ab[2x - 2x \ln x - x \ln^2 x]_1^\E = (\E - 2) \pi
			$$
		\end{itemize}
...
\begin{problem}
	...
	\begin{solution}
		$$
		\begin{aligned}
			V & = \frac{2\pi}{3} \int_0^\pi \ab(a (1 - \cos \theta))^3 \sin \theta \,\dif \theta \\
			& = \frac{2\pi}{3} \int_0^\pi a^3 (1 - \cos \theta)^3 \,\dif (1 - \cos \theta) \\
...
\begin{problem}
	...
	\begin{solution}
		...
		$$
		\begin{aligned}
			V & = \int_{-a}^a \pi b c \ab(1 - \frac{x^2}{a^2}) \,\dif x = \pi b c \int_{-a}^a \ab(1 - \frac{x^2}{a^2}) \dif x \\
...
\begin{problem}
	...
	\begin{solution}
		\begin{itemize}
			...
			$$
			\begin{aligned}
				A & = 2\pi \int_{-\pi/2}^{\pi/2} 2 \cos t \sqrt{4 \sin^2 t + \cos^2 t} \,\dif t \\
				& = 4\pi \int_{-\pi/2}^{\pi/2} \sqrt{3 \sin^2 t + 1} \,\dif (\sin t) \\
				& = 4\sqrt{3} \pi \int_{-1}^1 \sqrt{u^2 + \frac{1}{3}} \,\dif u \\
			...
			$$
			\begin{aligned}
				A & = 2\pi \int_0^2 x \sqrt{1 + \ab(\frac{x}{2})^2} \,\dif x \\
				& = \pi \int_0^2 \sqrt{1 + \frac{x^2}{4}} \,\dif (x^2) \\
				& = \frac{\pi}{2} \int_0^4 \sqrt{4 + u} \,\dif u \\
...
\begin{problem}
	...
	\begin{solution}
		...
		\begin{itemize}
			\item 对于切线形成的面积：
			$$
			A_1 = 2\pi \int_0^2 \frac{1}{2} x \sqrt{1 + \ab(\frac{1}{2})^2} \,\dif x = \frac{\sqrt{5} \pi}{2} \int_0^2 x \,\dif x = \sqrt{5} \pi
			$$

			\item 对于曲线形成的面积：
			$$
			\begin{aligned}
				A_2 & = 2\pi \int_1^2 \sqrt{x - 1} \sqrt{1 + \ab(\frac{1}{2 \sqrt{x - 1}})^2} \,\dif x \\
				& = \pi \int_1^2 \sqrt{4x - 3} \,\dif x \\
...
\begin{problem}
	...
	\begin{solution}
		\begin{itemize}
			...
			$$
			\begin{aligned}
				A & = 2\pi \int_0^\pi r \sin \theta \cdot \sqrt{r^2 + r'^2} \,\dif \theta \\
				& = 2\pi \int_0^\pi 2 a^2 (1 - \cos \theta) \sin \theta \sin \frac{\theta}{2} \,\dif \theta \\
				& = 2\pi \int_0^\pi 2 a^2 \cdot 2 \sin^2 \frac{\theta}{2} \cdot 2 \sin \frac{\theta}{2} \cos \frac{\theta}{2} \cdot \sin \frac{\theta}{2} \,\dif \theta \\
				& = 32\pi a^2 \int_0^\pi \sin^4 \frac{\theta}{2} \,\dif\ab(\sin \frac{\theta}{2}) \\
			...
			$$
			\begin{aligned}
				A & = 2 \cdot 2\pi \int_0^{\pi/4} r \sin \theta \cdot \sqrt{r^2 + r'^2} \,\dif \theta \\
				& = 4\pi \int_0^{\pi/4} a \sqrt{\cos 2\theta} \sin \theta \sqrt{a^2 \cos 2\theta + \frac{a^2 \sin^2 2\theta}{\cos 2\theta}} \,\dif \theta \\
				& = 4\pi a^2 \int_0^{\pi/4} \sin \theta \,\dif \theta \\
