...
\begin{theorem}[Holder 不等式]
	...
	$$
	\ab(\int_a^b f(x) g(x) \,\dif x)^2 \le \int_a^b f(x)^2 \,\dif x \int_a^b g(x)^2 \,\dif x
	$$

	\begin{proof}
		可以发现关于 $t$ 的方程
		$$
		\int_a^b \ab(t f(x) + g(x))^2 \,\dif x = t^2 \int_a^b f(x)^2 \,\dif x + 2t \int_a^b f(x) g(x) \,\dif x + \int_a^b g(x)^2 \,\dif x = 0
		$$
		是一定有解的，因此：
		$$
		\Delta = \ab(2 \int_a^b f(x) g(x) \,\dif x)^2 - 4 \int_a^b f(x)^2 \,\dif x \int_a^b g(x)^2 \,\dif x \ge 0
		$$
		于是得证。
	\end{proof}
\end{theorem}
...
\begin{theorem}[积分第一中值定理]
	...
	$$
	\exists\,\xi \in [a, b]: \int_a^b f(x) g(x) \,\dif x = f(\xi) \int_a^b g(x) \,\dif x
	$$
	\begin{proof}
		不妨设 $g(x) \ge 0$，那么有 $\int_a^b g(x) \,\dif x \ge 0$。设 $m \le f(x) \le M$。
		$$
		m \int_a^b g(x) \dif x \le \int_a^b f(x) g(x) \dif x \le M \int_a^b g(x) \dif x
		$$
		当 $g(x) = 0$ 时显然成立，否则就有：
		$$
		...
		$$
		f(\xi) = \frac{\int_a^b f(x) g(x) \,\dif x}{\int_a^b g(x) \,\dif x}
		$$
		于是得证。
	\end{proof}
\end{theorem}
...
$$
\exists\,\xi \in [a, b]: \int_a^b f(x) \,\dif x = f(\xi)(b - a)
$$
对于一般的可积函数 $f(x)$，我们称 $\frac{1}{b - a} \int_a^b f(x) \,\dif x$ 为 $f(x)$ 在 $[a, b]$ 上的平均值。
...
\begin{theorem}[积分第二中值定理]
	...
	\begin{itemize}
		...
		$$
		\exists\,\xi \in [a, b]: \int_a^b f(x) g(x) \,\dif x = g(a) \int_a^\xi f(x) \,\dif x
		$$
		\item 若 $g(x)$ 在 $[a, b]$ 上单调递增，且 $g(x) \ge 0$，那么：
		$$
		\exists\,\eta \in [a, b]: \int_a^b f(x) g(x) \,\dif x = g(b) \int_\eta^b f(x) \,\dif x
		$$
	\end{itemize}
\end{theorem}
...
\begin{theorem}[积分第三中值定理]
	...
	$$
	\exists\,\xi \in [a, b]: \int_a^b f(x) g(x) \,\dif x = g(a) \int_a^\xi f(x) + g(b) \int_\xi^b f(x) \,\dif x
	$$
\end{theorem}
...
$$
F(x) = \int_a^x f(t) \dif t \quad (x \in [a, b])
$$
那么我们称 $F(x)$ 为变上限积分。我们接下来对这种函数进行探究。

\begin{theorem}
	若 $f(t)$ 在 $[a, b]$ 上可积，那么 $F(x) = \int_a^x f(t) \,\dif t$ 在 $(a, b)$ 上连续。

	\begin{proof}
		我们需要证明，$\lim\limits_{h \to 0} F(x + h) = F(x)$。
		$$
		F(x_0 + h) - F(x_0) = \int_a^{x_0 + h} f(t) \,\dif t - \int_a^{x_0} f(t) \,\dif t = \int_{x_0}^{x_0 + h} f(t) \,\dif t
		$$
		因为 $f(t)$ 在 $[a, b]$ 有界，那么 $|f(t)| \le M$。于是：
		$$
		\ab|\int_{x_0}^{x_0 + h} f(t) \,\dif t| \le \left|\int_{x_0}^{x_0 + h} |f(t)| \,\dif t \right| \le M |h|
		$$
...
\begin{theorem}
	若 $f(t)$ 在 $[a, b]$ 上连续，那么 $F(x) = \int_a^x f(t) \dif t$ 在 $[a, b]$ 上可导，且 $F'(x) = f(x)$。那么：
	$$
	\Delta F = F(x + \Delta x) - F(x) = \int_x^{x + \Delta x} f(t) \dif t f(\xi) \Delta x
	$$
...
\begin{theorem}[原函数存在定理]
	若 $f(x)$ 在 $[a, b]$ 上连续，那么 $F(x) = \int_a^x f(x) \,\dif x$ 是 $f(x)$ 在 $[a, b]$ 上的一个原函数。
\end{theorem}

因此，初等函数均存在原函数，但是原函数不一定是初等函数。比如 $\int \frac{\sin x}{x} \,\dif x$。
...
\begin{theorem}[Newton-Leibniz 公式]
	...
	$$
	\int_a^b f(x) \,\dif x = F(b) - F(a) = \left. F(x) \right|_a^b
	$$
	\begin{proof}
...
\begin{problem}
	...
	\begin{solution}
		\begin{enumerate}
			...
			$$
			\begin{aligned}
				\text{原式} & = \int \frac{\dif (a \sin t)}{a^2 \sin^2 t \cdot a \cos t}  = \int \frac{a \cos t \,\dif t}{a^3 \sin^2 t \cos t} \\
				& = \int \frac{\dif t}{a^2 \sin^2 t} = -\frac{1}{a^2} \cot t + C \\
			...
			$$
			\begin{aligned}
				\text{原式} & = \int \frac{\dif (b \tan t)}{b^2 \tan^2 t \cdot b \sec t} = \int \frac{b \sec^2 t \,\dif t}{b^3 \tan^2 t \sec t} \\
				& = \frac{1}{b^2} \int \frac{\dif(\sin t)}{\sin^2 t} = -\frac{1}{b^2 \sin t} + C \\
			...
			$$
			\begin{aligned}
				\text{原式} & = \int \frac{\sinh x}{\cosh^2 x - 1} \,\dif x = \int \frac{\dif(\cosh x)}{\cosh^2 x - 1} \\
			...
			$$
			\begin{aligned}
				\text{原式} & = \int \frac{\dif(\arctan t)}{1 + t} = \int \frac{\dif t}{(1 + t)(1 + t^2)} \\
				& = \frac{1}{2} \int \ab(\frac{1}{1 + t} + \frac{1}{1 + t^2} - \frac{t}{1 + t^2}) \,\dif t \\
			...
			$$
			\begin{aligned}
				\text{原式} & = \int \frac{\dif x}{(1 + \sqrt{2} x + x^2)(1 - \sqrt{2} x + x^2)} \\
				& = \frac{1}{2} \int \ab(\frac{1 - \frac{1}{\sqrt{2}} x}{1 - \sqrt{2} x + x^2} - \frac{1 + \frac{1}{\sqrt{2}} x}{1 + \sqrt{2} x + x^2}) \,\dif x \\
			...
			$$
			\begin{aligned}
				\text{原式} & = \int \ab(\frac{1}{x^4} - \frac{1}{x^2} + \frac{1}{1 + x^2}) \,\dif x \\
			...
			$$
			\begin{aligned}
				\text{原式} & = \int \frac{x^4 + 1}{(x^2 + 1)(x^4 - x^2 + 1)} \,\dif x \\
				& = \int \frac{(x^4 - x^2 + 1) + x^2}{(x^2 + 1)(x^4 - x^2 + 1)} \,\dif x \\
				& = \int \frac{\dif x}{x^2 + 1} + \frac{1}{3} \int \frac{\dif (x^3)}{x^6 + 1} \\
...
\begin{problem}
	...
	\begin{solution}
		\begin{enumerate}
			...
			$$
			\begin{aligned}
				\text{原式} & = \int \frac{\dif(2 \arctan u)}{2 + \frac{2u}{1 + u^2}}= \int \frac{\frac{2}{1 + u^2}}{2 + \frac{2u}{1 + u^2}} \,\dif u \\
				& = \int \frac{\dif u}{(1 + u^2)} = -\frac{1}{1 + u} + C \\
			...
			\item[\textbf{4)}]
			$$
			\text{原式} = \int \frac{x}{x^2 \sqrt{x^2 - 1}} \,\dif x + \int \frac{\dif x}{x^2 \sqrt{x^2 - 1}}
			$$
			其中：
			$$
			\begin{aligned}
				\int \frac{x}{x^2 \sqrt{x^2 - 1}} \,\dif x & = \frac{1}{2} \int \frac{\dif (x^2)}{x^2 \sqrt{x^2 - 1}} \\
				& = \frac{1}{2} \int \frac{\dif (u^2 + 1)}{u (u^2 + 1)} = \int \frac{\dif u}{u^2 + 1} \\
			...
			$$
			\begin{aligned}
				\int \frac{1}{x^2 \sqrt{x^2 - 1}} & = \int \frac{\dif(\csc t)}{\csc^2 t \cot t} \\
				& = \int \frac{-\cot t \csc t}{\csc^2 t \cot t} \,\dif t = -\int \sin t \,\dif t \\
...
\begin{problem}
	...
	\begin{solution}
		\begin{enumerate}
			...
			$$
			\begin{aligned}
				\int_0^1 x \,\dif x & = \lim_{n \to \infty} \sum_{i=1}^n \frac{i}{n} \cdot \frac{1}{n} \\
			...
			$$
			\begin{aligned}
				\int_a^b \frac{\dif x}{x^2} & = \lim_{n \to \infty} \sum_{i=1}^n \frac{1}{\ab(a + \frac{iL}{n})^2} \cdot \frac{L}{n} \\
			...
			从而得到：
			$$
			\int_a^b \frac{\dif x}{x^2} = \frac{1}{a} - \frac{1}{a + L} = \frac{1}{a} - \frac{1}{b}
			$$
		\end{enumerate}
	\end{solution}
\end{problem}
...
\begin{problem}
	...
	\begin{solution}
		\begin{enumerate}
			...
			$$
			\begin{aligned}
				\int_0^{\frac{\pi}{2}} \sin^n x \,\dif x & = \int_0^{\frac{\pi - \varepsilon}{2}} \sin^n x \,\dif x + \int_{\frac{\pi - \varepsilon}{2}}^{\frac{\pi}{2}} \sin^n x \,\dif x \\
			...
			$$
			\int_0^{\frac{\pi}{2}} \sin^n x \,\dif x < \varepsilon
			$$
			因此得到 $\lim\limits_{n \to 0} \int_0^{\frac{\pi}{2}} \sin^n x \,\dif x = 0$。

			\item[\textbf{4)}]
			$$
			\lim_{n \to \infty} \abs*{\int_n^{n+1} \frac{\cos x}{x} \,\dif x} \le \lim_{n \to \infty} \int_n^{n+1} \abs*{\frac{\cos x}{x}} \,\dif x \le \lim_{n \to \infty} \abs*{\frac{1}{n + 1}} = 0
			$$
			因此
			$$
			\lim_{n \to \infty} \int_n^{n+1} \frac{\cos x}{x} \,\dif x = 0
			$$
		\end{enumerate}
	\end{solution}
\end{problem}
...
\begin{problem}
	...
	\begin{proof}
		\begin{enumerate}
			...
			$$
			\begin{gathered}
				f\ab(\frac{\pi}{2}) < \int_0^1 f(x) \,\dif x < f(0) \\
				\frac{2}{\pi} < \int_0^1 f(x) \,\dif x < 1
			\end{gathered}
			$$
		\end{enumerate}
	\end{proof}
...
\begin{problem}
	...
	\begin{proof}
		$$
		\begin{aligned}
			& & \int_0^\beta f(x) \,\dif x & \ge \beta \int_0^1 f(x) \,\dif x \\
			\Leftrightarrow & & \int_0^\beta f(x) \,\dif x & \ge \beta \int_0^\beta f(x) \,\dif x + \beta \int_\beta^1 f(x) \,\dif x \\
			\Leftrightarrow & & (1 - \beta) \int_0^\beta f(x) \,\dif x & \ge \beta \int_\beta^1 f(x) \,\dif x \\
			\Leftrightarrow & & \frac{1}{\beta} \int_0^\beta f(x) \,\dif x & \ge \frac{1}{1 - \beta} \int_\beta^a f(x) \,\dif x \\
		\end{aligned}
		$$
		根据积分第一中值定理：
		$$
		\exists\,\xi \in (0, \beta), \exists\,\eta \in (\beta, 1): \frac{1}{\beta} \int_0^\beta f(x) \,\dif x = f(\xi),\ \frac{1}{1 - \beta} \int_\beta^1 f(x) \,\dif x = f(\eta)
		$$
...
\begin{problem}
	...
	\begin{proof}
		\begin{enumerate}
			...
			$$
			\ab(\int_a^b A(x) \,\dif x)^2 + \ab(\int_a^b B(x) \,\dif x)^2 \le (b - a) \int_a^b \ab(A(x)^2 + B(x)^2) \,\dif x
			$$
			...
			$$
			\ab(\int_a^b f(x) \sin nx \,\dif x)^2 + \ab(\int_a^b f(x) \cos nx \,\dif x)^2 \le \ab(\int_a^b f(x) \,\dif x)^2
			$$
		\end{enumerate}
	\end{proof}
\end{problem}